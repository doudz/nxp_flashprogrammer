...
# Compiled at: 2012-11-15 17:10:27
import sys, ZedSerialProtocol, ZedSerialEnum, struct, time
try:
    ...
    class cZedSerialApi():
        ...
        def __init__(self, iBaudrate, sComPort):
            aMode = [
             iBaudrate, 'N', 8, 1]
            self.oZsp = ZedSerialProtocol.cZedSerialProtocol(sComPort, aMode)
            self.bDebug = False
        ...
        def mSendMsg(self, sOutMsg, sName):
            if self.bDebug:
                ...
                print('%s.%s' % (sName, ZedSerialEnum.SerProtCodeStr(iReqType)))
        ...
        def mReceiveMsg(self, sName):
            ...
            if self.bDebug:
                ...
                if iStatus == ZedSerialEnum.OK:
                    print('%s.%s' % (sName, ZedSerialEnum.SerProtCodeStr(iRspType)))
                else:
                    print('%s.Rcv => %s' % (sName, ZedSerialEnum.StatusStr(iStatus)))
            return (iRet, sInMsg)
        ...
        def mCheckData(self, sData, iResponse, sName):
            if len(sData) >= 2:
                ...
                if iFrameType != iResponse:
                    if self.bDebug:
                        print('%s invalid response' % sName)
                    return ZedSerialEnum.INVALID_RESPONSE
                elif iStatus != 0:
                    if self.bDebug:
                        print('%s response error %s(%d)' % (sName, ZedSerialEnum.StatusStr(iStatus), iStatus))
                    return iStatus
                else:
                    return ZedSerialEnum.OK
            else:
                if self.bDebug:
                    print('%s invalid length (%d)' % (sName, len(sData)))
                return ZedSerialEnum.INVALID_RESPONSE
        ...
        def ZedDetectProcessorVersion(self):
            ...
            if u32ChipId == JN5139v0_CHIP_ID:
                ...
                if self.iProcessorPartNo == 4:
                    if self.iProcessorRevNo == 1 and self.iMaskNo == 1:
                        eReadResult, abProcVer = self.ZedRAMRead(4, 4)
                        if eReadResult == ZedSerialEnum.OK:
                            u32RomId = struct.unpack('>L', abProcVer)[0]
                            self.iMaskNo = u32RomId == 33947702 and 3
            if self.bDebug:
                print('ProcPartNo = %d' % self.iProcessorPartNo)
                print('ProcRevNo  = %d' % self.iProcessorRevNo)
                print('ProcMaskNo  = %d' % self.iMaskNo)
            return ZedSerialEnum.OK
        ...
        def ZedRegisterRead(self, iRegAddr):
            ...
            if self.iProcessorPartNo == PROC_PARTNO_JN5121:
                ...
                if self.iProcessorPartNo == PROC_PARTNO_JN5139:
                    iRet, abReadValue = self.iProcessorRevNo == 0 and self.ZedRAMRead(iRegAddr, 4)
                    if iRet == ZedSerialEnum.OK:
                        u32RegVal = struct.unpack('<L', abReadValue)[0]
                        return (
                         iRet, u32RegVal)
        ...
        def ZedFlashSelectType(self, eFlashType, u32Address=None):
            ...
            if eFlashType == E_FL_CHIP_CUSTOM:
                if u32Address is None:
                    return ZedSerialEnum.NOT_SUPPORTED
            else:
                u32Address = 0
        ...
        def ZedLoginIn(self, aPassKey):
            if self.iProcessorPartNo == 4:
                eReadResult, abMACAddress = self.iProcessorRevNo == 0 and self.ZedFlashRead(48, 8)
            else:
                if self.iProcessorPartNo == 4:
                    if self.iProcessorRevNo == 1:
                        eReadResult, abMACAddress = self.ZedRAMRead(JAGUAR_EFUSE_LOCATION, 8)
                        bGetMacAddrFromFlash = abMACAddress == struct.pack('LL', 0, 0) and True
    ...
    def test(sComPort='COM2', iMaxOffset=92160):
        ...
        if bReadFlashId:
            ...
            if iManId == 31 and iDevId == 96:
                ...
                print(bWalkFlash and 'Erasing flash')
                print(SerAPI.ZedFlashBulkErase())
